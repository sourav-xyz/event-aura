...
export default function AdminDashboard() {
  ...
  const fetchStats = async () => {
    try {
      ...
      setStats({
        totalOrders: s.total || response.total || 0,
        totalRevenue: s.totalRevenue || 0,
        totalUsers: response.totalUsers || 0, // 👈 important
        totalEvents: 0,
        pendingOrders: s.pending || 0,
        confirmedOrders: s.confirmed || 0,
                ...
                <tbody>
                  {stats.recentOrders.map((order) => (
                    ...
                    >
                      ...
                      <td className="py-3 px-2">
                        <span
                          className={`inline-block px-2 py-1 rounded-full text-xs font-medium capitalize ${getStatusColor(order.status)}`}
                        >
                          {order.status}
                        </span>
                      </td>
